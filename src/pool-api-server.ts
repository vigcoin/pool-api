...
export class Server {
  ...
  async onAddressStats(req: Request, res: Response) {
    ...
    if (longpoll === 'true') {
      let exist = await this.api.getAddressExistence(
        this.redis,
        this.config.coin,
        address
      );
    ...
    let status = await this.api.getAddressStatus(
      this.redis,
      this.config.coin,
      address
    );
    let payments = await this.api.getAddressPayments(
      this.redis,
      this.config.coin,
      address
    );
    let charts = await this.charts.getUserCharts(
      this.redis,
      this.config.coin,
      address
    );
