...
test('Should app address stats 2', () => {
  return request(app)
    .get('/stats_address')
    .query({ address: 'aaa', longpoll: false })
    .expect(200);
});
...
test('Should app address stats 2', () => {
  return request(app)
    .get('/stats_address')
    .query({ address: 'aaa', longpoll: true })
    .expect(200);
});
...
test('Should app address stats 2', () => {
  return request(app)
    .get('/stats_address')
    .query({ address: 'uuuu', longpoll: true })
    .expect(200);
});
...
test('Should get payments address', () => {
  return request(app)
    .get('/get_payments')
    .query({ address: 'aaa', time: Date.now() })
    .expect(200);
});
...
test('set blocks data', async () => {
  ...
  await zadd(config.coin + ':blocks:matured', 1, [1, 2, 3, 4, 1].join(':'));

  await zadd(config.coin + ':blocks:matured', 1, [1, 2, 3, 4, 1, 3].join(':'));
...
test('Should get admin log  none exist', () => {
  return request(app)
    .get('/admin_log')
    .query({ password: 1234, file: 'aaa.txt' })
    .expect(403);
});
...
test('Should get admin log', done => {
  ...
  file.end(() => {
    request(app)
      .get('/admin_log')
      .query({ password: 1234, file: 'bbb.log' })
      .expect(200)
      .then(() => {
        done();
      });
